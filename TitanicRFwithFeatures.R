...
rfFit <- randomForest(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + 
...
predtest <- predict(rfFit, newdata=test,type="class")

solution <- read.csv("test.csv",na.strings = c("NA",""))
solution$Survived <- predtest
solution<-solution[,c(1,12)]

write.csv(solution,"TitanicSolutionRF.csv",row.names = FALSE)
